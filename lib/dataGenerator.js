...
/*
 * Класс DataGenerator - генератор тестовых данных
 * Позволяет генерировать данные по заданной схеме с различными типами полей
 */
class DataGenerator {
  /**
  ...
  generate(schema, count = 1) {
    if (!schema || typeof schema !== 'object') throw new TypeError('schema должен быть объектом');
    if (!Number.isInteger(count) || count < 1) throw new TypeError('count должен быть положительным целым числом');
   ...
   * @returns {Object} - сгенерированный объект
   */
  _generateItem(schema, depth = 0) {
    // Защита от слишком глубокой рекурсии
    if (depth > this.maxDepth) throw new Error('Слишком глубокая схема');
    ...
    for (const [key, cfg] of Object.entries(schema)) {
      ...
      if (!faker) throw new TypeError(`Неизвестный тип "${config.type}" для ключа "${key}"`);
   ...
   * @returns {boolean} - сгенерированное логическое значение
   */
  _generateBoolean() {
   ...
   * @param {Object} opts - опции генерации
   * @returns {string} - сгенерированная дата в ISO-формате
   */
  _generateDate(opts = {}) {
  ...
  _generateArray(opts = {}, depth = 0) {
    ...
    for (let i = 0; i < size; i++) {
      if (typeof itemType === 'string') {
        ...
        if (!faker) throw new TypeError(`Неизвестный тип элемента массива "${itemType}"`);
